...
model{
    ...
    for ( i in 1:4993 ) DS_part_2[i] ~ ordered_probit( eta[i] , cutpoints );
}
generated quantities{
     vector[N] log_lik;
    for ( i in 1:N ) {
        log_lik[i] = ordered_probit_lpmf(DS_part_2[i] | b0[DS_part_0[i]] * lnIM_part_2[i] + b1 * sum(append_row(0, delta)[1:DS_part_0[i]]) + b2 * ht_part_0[i], cutpoints);
    }
}
