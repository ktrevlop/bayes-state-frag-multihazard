...
generated quantities{
  vector[N] log_lik;
  for ( i in 1:N ) {
    log_lik[i] = ordered_probit_lpmf(DS_part_2[i] | b0[DS_part_0[i]] * lnIM_part_2[i] + b1 * sum(append_row(0, delta)[1:DS_part_0[i]]) + b2[DS_part_0[i]] * exp(lnDeltaT_part_1[i]), cutpoints);
  }
}
