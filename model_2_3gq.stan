...
model{
  ...
  b0 ~ normal( 4 , 2 );
  for ( i in 1:N ) {
    eta[i] = b0 * lnIM_part_2[i] + b1 * sum(append_row(0, delta)[1:DS_part_0[i]]);
  }
...
generated quantities{
  vector[N] log_lik;
  for ( i in 1:N ) {
    log_lik[i] = ordered_logistic_lpmf(DS_part_2[i] | b0 * lnIM_part_2[i] + b1 * sum(append_row(0, delta)[1:DS_part_0[i]]), cutpoints);
  }
}
