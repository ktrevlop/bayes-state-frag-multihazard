...
n_ds <- length(grep("^cutpoints\\[", model_2.5b_par$variable)) + 1
...
b0 <- model_2.5b_par$mean[1]
b1 <- model_2.5b_par$mean[2]
cutpoints <- model_2.5b_par$mean[3:6]
...
fragility_model_2.5b_lo <- fragility_curves |> mutate(DS = factor(DS)) |> 
  mutate(Model = "Model 2.5b lo")
...
fragility_model_2.5b_up <- fragility_curves |> mutate(DS = factor(DS)) |> 
  mutate(Model = "Model 2.5b up")




#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Fragility Model 2.6b ----
...
# The number of cutpoints and damage states in the model
n_ds <- length(grep("^cutpoints\\[", model_2.6b_par$variable)) + 1
...
b0 <- model_2.6b_par$mean[1]
b1 <- model_2.6b_par$mean[2]
cutpoints <- model_2.6b_par$mean[3:6]
...
fragility_model_2.6b_lo <- fragility_curves |> mutate(DS = factor(DS)) |> 
  mutate(Model = "Model 2.6b lo")
...
fragility_model_2.6b_up <- fragility_curves |> mutate(DS = factor(DS)) |> 
  mutate(Model = "Model 2.6b up")




#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Fragility Model 2.7b ----

# This Model has one surface for each damage state.
# Fix one predictor for the upper and lower bound.

# The number of cutpoints and damage states in the model
n_ds <- length(grep("^cutpoints\\[", model_2.7b_par$variable)) + 1
...
b0 <- model_2.7b_par$mean[1]
b1 <- model_2.7b_par$mean[2]
cutpoints <- model_2.7b_par$mean[3:6]
...
fragility_model_2.7b_lo <- fragility_curves |> mutate(DS = factor(DS)) |> 
  mutate(Model = "Model 2.7b lo")
...
fragility_model_2.7b_up <- fragility_curves |> mutate(DS = factor(DS)) |> 
  mutate(Model = "Model 2.7b up")
...
sdfc <- bind_rows(
  fragility_model_2.5b_lo,
  fragility_model_2.5b_up,
  fragility_model_2.6b_lo,
  fragility_model_2.6b_up,
  fragility_model_2.7b_lo,
  fragility_model_2.7b_up
)
...
figure_compare <- ggplot(
  ...
  scale_linetype_manual(
    values =
      setNames( c("dotted", "dotted", "dashed", "dashed", "solid", "solid"),
                unique(sdfc$Model) ) )+
  facet_grid(
    cols = vars(DS),
    rows = vars(DS_part_0),
...
ggsave(
  plot = figure_compare,
  paste0("./data_store/fragility_models/compare_models_2.5b_2.6b_2.7b.png"),
  width = 29.7,
  height = 21.0,
  units = c("cm"),
  dpi = 300,
